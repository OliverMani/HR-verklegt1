...
V - Notanlegar skipanir: "L" "R" "x" "x"
    ...
    "" =
        Info:
            ...
            eftir WorkRequestID (Ef þú vilt skoða verkbeiðni númer 3 þá ýtir þú á "L" og 
            síðan "3"). Þegar þú ert búin/n að leita þá kemur fyrst upp upplýsingar um 
...
VS - Er notanlegt í (F)asteign, (V)erkbeiðnir og (S)tarfsmenn
     Notanlegar skipanir: "x" "x" "x" "x" "x"
        Info:
            ...
            inni í (F)asteign og síðan skrifa fasteignarID-ið og "VS", þá birtast allar 
            skýrslur varðandi þessa fasteign. Það er einnig hægt að nota "VS" í 
            (S)tarfsmenn. Þar skrifar þú starfsmannsID og "VS" fyrir aftan, þá færðu 
            verksýrslurnar sem þessi starfsmaður hefur skráð.

C - Er Notanlegt í (S)tarfsmenn, (V)erkbeiðnir og (F)asteign
        Info:
            Býr til nýjan hlut í ákveðinni skrá og það fer eftir því hvað þú valdir 
...
L - Er notanlegt í (S)tarfsmenn, (V)erkbeiðnir, (F)asteign og "VS"
        Info:
            Þú notar þessa skipun til að leita í kerfinu eftir einhverju sérstöku og 
            það fer eftir á hvaða skjá þú varst síðast á. Ef þú varst síðast á 
            (S)tarfsmenn og skrifar "C", þá býrðu til nýjan starfsmann.

R - Er notanlegt í (S)tarfsmenn, (V)erkbeiðnir, (F)asteign
        Info:
            Þú notar þessa skipun til að raða hlutum eftir áfangastað, eins og í 
            (S)tarfsmenn, þar er hægt að raða starfsmönnum eftir áfangastað og í 
            (F)asteign er hægt að raða öllum fasteignum sem eru á ákveðnum áfangastað.

A - Er notanlegt í (V)erkbeiðnir, (F)asteign og (S)tarfsmenn
        Info:
            Þessi skipun sýnir lista yfir öllu sem hægt er að sjá varðandi ákveðin 
            hlut. Að nota þessa skipun í (F)asteign skilar öllum fasteignum sem 
            fyrirtækið hefur á skrá.
"""
